...
(defun get-dfun-constructor (generator &rest args)
  #!+sb-doc
  "Returns a function generated by calling GENERATOR with arguments
ARGS.  Caches the result in *DFUN-CONSTRUCTORS* unless
*ENABLE-DFUN-CONSTRUCTOR-CACHING* is null.

If *RAISE-METATYPES-TO-CLASS-P* is T and GENERATOR is one of
EMIT-CHECKING, EMIT-CACHING, EMIT-IN-CHECKING-CACHE-P, or
EMIT-CONSTANT-VALUE, then the first argument to the generator (the
metatypes argument) is processed such that each element is either T or
'CLASS."
  ;;; Returns a function that returns a function that performs
  ;;; something like a slot read/write/boundp.  Typical values of
  ;;; generator are EMIT-ONE-CLASS-READER and other EMIT-* symbols.

  ;;; The returned function takes closure arguments according to the
  ;;; particular generator used.
  ...
  (when (and *raise-metatypes-to-class-p*
    ...
    (destructuring-bind (metatypes &rest rest-generator-args)
        args
...
(defun make-one-class-accessor-dfun (gf type wrapper index)
  #!+sb-doc
  "Returns a function to be used as the accessor function of type
TYPE (one of 'reader 'boundp 'writer) for the slot at index INDEX with
the generic function GF and wrapper WRAPPER."
  (let ((emit (ecase type
